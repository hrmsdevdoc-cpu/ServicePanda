...
class NotificationService {
  ...
  private convertActivitiesToNotifications(serviceRequests: any[]): Notification[] {
    ...
    const notifications = serviceRequests.slice(0, 10).map((request, index) => {
      ...
      return {
        ...
        message: this.formatServiceRequestMessage(request),
        type: this.getServiceRequestNotificationType(request.status),
        isRead: isRead,
  ...
  private formatServiceRequestMessage(request: any): string {
  ...
  private getServiceRequestTitle(request: any): string {
    const serviceType = request.serviceType || request.categoryName || 'Service';
    return `${serviceType} Request Update`;
  }

  private getServiceRequestNotificationType(status: string): 'info' | 'success' | 'warning' | 'error' {
    switch (status) {
      case 'completed':
        return 'success';
      case 'cancelled':
