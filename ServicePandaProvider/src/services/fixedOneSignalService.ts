...
// OneSignal SDK - COMPLETELY DISABLED to prevent NativeEventEmitter crash
// For iOS: OneSignal is initialized natively in AppDelegate.mm
// This service now works in "native-only" mode - NO JAVASCRIPT SDK
let OneSignal: any = null;

console.log('🔔 FixedOneSignal Service: Native-only mode (JS SDK DISABLED)');
...
class FixedOneSignalService {
  ...
  async initialize(forceReinit: boolean = false) {
    ...
    try {
      ...
      // iOS uses native OneSignal SDK in AppDelegate; skip JS SDK entirely
      if (Platform.OS === 'ios') {
        console.log('🍎 iOS detected: using native OneSignal SDK; skipping JS SDK init');
        this.isInitialized = true;
        // No registration via REST or JS on iOS to avoid duplicates
        return;
      }

      // Step 1: Request notification permissions FIRST (Android)
      await this.requestNotificationPermissions();

      // Step 2: Initialize OneSignal SDK properly (Android only)
  ...
  private async initializeOneSignalSDK() {
    if (!OneSignal) {
      console.log('⚠️ OneSignal SDK not available; skipping JS initialization');
      return;
    }
