...
class WorkingAndroidNotificationService {
  ...
  initialize() {
    ...
    try {
      // Dynamically load PushNotification only on Android
  ...
  clearAllNotifications() {
    try {
      if (!this.PushNotification) { this.PushNotification = require('react-native-push-notification'); }
  ...
  checkPermissions(callback: (permissions: any) => void) {
    try {
      if (!this.PushNotification) { this.PushNotification = require('react-native-push-notification'); }
      this.PushNotification?.checkPermissions?.(callback);
    } catch { }
  }
}
